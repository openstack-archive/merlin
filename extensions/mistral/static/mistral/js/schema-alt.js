...
//  });
      ...
      var panelsMixin = Barricade.Blueprint.create(function (schema) {
        ...
        this.addPanel = function(barricadeContainer, itemID) {
          var item = barricadeContainer.getByID(itemID),
            panel = Object.create(panelProto).create(item, barricadeContainer);
          // insert new actions before workflows
          if ( item.instanceof(types.Mistral.Action) ) {
            for ( var i = 0; i < panels.length; i++ ) {
              var curObj = panels[i]._barricadeObj;
              if ( curObj && curObj.instanceof(types.Mistral.Workflow) ) {
                break;
              }
            }
            panels.splice(i, 0, panel);
          } else {
            panels.push(panel);
          }
        };
