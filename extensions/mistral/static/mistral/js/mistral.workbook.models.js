      ...
      function(fields, panel, utils, $http, $q) {
        ...
        function TaskFactory(json, parameters) {
          var type = json.type || 'action',
            wfType = parameters.wfType || 'direct',
            wfTaskType = taskTypes[wfType],
            taskType = wfTaskType.extend({}, taskTypes[type]);

          return taskType.create(json, parameters);
        }
                ...
                self.on('childChange', function(child, op) {
                  if ( op === 'taskType' ) {
                    var taskId = child.getID(),
                      ...
                      params = utils.extend(parameters, {id: taskId});
        ...
        models.Workbook = fields.frozendict.extend({
          ...
          'workflows': {
            '@class': fields.dictionary.extend({
              create: function(json, parameters) {
                var params = utils.extend(parameters, {wfType: 'direct'}),
                  self;
                self = fields.dictionary.create.call(this, json, params);
                self.on('childChange', function(child, op) {
                  if ( op === 'workflowType' ) {
